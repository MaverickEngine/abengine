...
declare var abengine_powerwords_api: any;

export default class PowerWordsScorer implements IABEngineScorer {
    powerword_list: string[] = [];
    // powerword_regex: RegExp;

    async init() {
        // if (this.powerword_list.length) return; // Cached
        this.powerword_list = await jQuery.get(abengine_powerwords_api)
        .catch(err => {
    ...
    score(headline: string) {
        if (!this.powerword_list.length) return {
            name: "Powerwords",
            score: 0,
            message: `No Powerwords set - please set some Powerwords in the <a href="/wp-admin/admin.php?page=abengine">settings page</a>`,
            pass: false
        };
