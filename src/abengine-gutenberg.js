import "./abengine-gutenberg.scss";
import Calc_Score from "./abengine-score.ts";
...
async function main() {
    ...
    async function display_analysis(container, title_descriptor) {
        ...
        <div class='abengine-score' style="background-color: rgba(${ colour.join(", ")}, 0.6)">
            <div class='abengine-score-value'>${ Math.floor(scores.total_score * 100) }<div class='abengine-divisor'>100</div></div>
            <div class='abengine-score-title'>ABEngine<br>Score</div>
        </div>`);
        container.html(score_el);
        const score_analisys_container = jQuery(`<div class="abengine-analysis"></div>`);
        ...
        // const analysis = jQuery(`<div class="abengine-analysis">
        //     <div class="abengine-analysis-readability">Readability: ${score.readability.message} (${Math.round(score.readability.ease_score)})</div>
        //     <div class="abengine-analysis-length">Length: ${score.length.message} (${score.length.length})</div>
        //     <div class="abengine-analysis-powerwords">Powerwords: ${(score.powerwords.words.length) ? score.powerwords.words.map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(", ") : "None" }</div>
        // </div>`);
    ...
    jQuery(async () => {
        ...
        const headline_score_container_el = jQuery("<div id='abengine-score-container'></div>");
